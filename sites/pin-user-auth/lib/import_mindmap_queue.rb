class ImportMindmapQueue
  INPUT_QUEUE = "import_mindmap_input_queue"
  COMPLETE_QUEUE = "import_mindmap_complete_queue"
  ...
  # 从 input_queue 取出一个任务进行处理
  def process_task
    qid = @input_queue.pop
    return false if qid.nil?
    info = @info_hash.get(qid)
    p "处理 #{qid} => #{info}"
    # 处理这个任务
    _info = create_mindmap_and_image_and_build_complete_info_hash(info)

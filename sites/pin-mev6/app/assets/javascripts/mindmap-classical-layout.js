...
jQuery.extend(pie.mindmap, {
  ...
  do_layout_classical : function(R){    
		...
		R.canvas_elm.scrollLeft(paper_width  + root.width /2 - R.canvas_elm.width() /2);
		R.canvas_elm.scrollTop( paper_height + root.height/2 - R.canvas_elm.height()/2);    
      ...
      if(node.closed){
        ...
        node.ch_pos(real_left, top);
	      ...
	      jQuery.each(children, function(index, child){
				  var new_left = real_left - R.options.NODE_X_GAP;
    ...
    var _r_for_right = function(node, left, top){
      ...
      var children = node.children;
        ...
        node.ch_pos(real_left, top);
	      ...
	      var _tmp_height = node.children_box_top_offset();
	      jQuery.each(children, function(index, child){
				  var new_left = real_left + node.width + R.options.NODE_X_GAP;
					var new_top  = top + _tmp_height

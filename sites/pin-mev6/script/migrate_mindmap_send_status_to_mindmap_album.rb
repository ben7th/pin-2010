...
mindmaps.each_with_index do |m,i|
  ...
  begin
    ...
    next if status == "public"
    
    
    ma = MindmapAlbum.find_or_create_by_user_id_and_title_and_send_status(mindmap.user.id,"私有导图","private")
    ...
    l = Logger.new("/web/2010/logs/migrate_mindmap_send_status_to_mindmap_album.log")
    l.error(ex.message)

...
Mindpin.CollectorClip = {
  ...
  init: function(){
    this.document = getWebWindow().document;
  ...
  _make_coverlayer_dom:function(){
    ...
    coverlayer_dom.setAttribute("style","background-color:#FF0000;position:fixed;opacity:1;");
...
//      char_number += ($(el).text().length-2); // 减去2（取消 二字）
    ...
    if(number==0){button.attr("label","发送捕捉到的元素")}
  },
  ...
  _do_clip:function(evt){
    ...
    choose_div.setAttribute("style","background-color:#74DF00;position:absolute;opacity:0.9;width:"+el.outerWidth()+";height:"+el.outerHeight()+";top:"+el.offset().top+";left:"+el.offset().left)
    choose_div.setAttribute("class","choosed_element");
    var scroll_top = jQuery(getWebWindow().document,getWebWindow()).scrollTop();
    $(choose_div).css({
      "width":el.outerWidth()+"px",
      "height":el.outerHeight()+"px",
      "top":el.offset().top+scroll_top+"px",

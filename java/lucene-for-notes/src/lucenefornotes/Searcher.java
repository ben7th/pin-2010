...
public class Searcher {
  ...
  private String result(ScoreDoc[] hits, IndexSearcher is, long time, Query query) throws CorruptIndexException, IOException, InvalidTokenOffsetsException {
    // 输出搜索出的文件名
    String[][] arrays = new String[hits.length][3];
    for (int i = 0; i < hits.length; i++) {
      ...
      String commitid = doc.get("commitid");
      String highStr = "";
      if (content!=null) {
  ...
  private String resultXML(String[][] arrays, long time) {
    ...
    sb.append("<search_results time='" + time + "' count='"+arrays.length+"'>");
    for (int i = 0; i < arrays.length; i++) {
      sb.append("<search_result>");
      sb.append("<path>" + arrays[i][0] + "</path>");
      sb.append("<highlight> <![CDATA[" + arrays[i][1] + "]]> </highlight>");
      sb.append("<commit_id>" + arrays[i][2] + "</commit_id>");
      sb.append("</search_result>");
    }

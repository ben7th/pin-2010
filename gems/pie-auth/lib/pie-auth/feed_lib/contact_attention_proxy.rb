class ContactAttentionProxy
  ...
  module Followings
    ...
    def followings_contacts
      _followings_contacts_id_list.map { |id|
        Contact.find_by_id(id)
      }.select{|c|!EmailActor.get_user_by_email(c.email).blank?}.compact
    end
  ...
  module Fans
    ...
    # 读缓存或者读数据库
    def fans_contacts
      _fans_contacts_id_list.map { |id|
        Contact.find_by_id(id)
      }.select{|c|!c.user.blank?}.compact
    end

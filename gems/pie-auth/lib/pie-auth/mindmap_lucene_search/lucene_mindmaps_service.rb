...
module LuceneMindmapsService
  class Client
    ...
    def index_one_mindmap(mindmap_id)
      send_index_one_mindmap(mindmap_id)
      return recv_index_one_mindmap()
    end

    def send_index_one_mindmap(mindmap_id)
      send_message('index_one_mindmap', Index_one_mindmap_args, :mindmap_id => mindmap_id)
    end

    def recv_index_one_mindmap()
      result = receive_message(Index_one_mindmap_result)
      return result.success unless result.success.nil?
      raise ::Thrift::ApplicationException.new(::Thrift::ApplicationException::MISSING_RESULT, 'index_one_mindmap failed: unknown result')
    end
    ...
    def send_delete_index(mindmap_id)
      send_message('delete_index', Delete_index_args, :mindmap_id => mindmap_id)
    end
  ...
  class Processor
    ...
    def process_index_one_mindmap(seqid, iprot, oprot)
      args = read_args(iprot, Index_one_mindmap_args)
      result = Index_one_mindmap_result.new()
      result.success = @handler.index_one_mindmap(args.mindmap_id)
      write_result(result, oprot, 'index_one_mindmap', seqid)
    end

    def process_delete_index(seqid, iprot, oprot)
      args = read_args(iprot, Delete_index_args)
      result = Delete_index_result.new()
      result.success = @handler.delete_index(args.mindmap_id)
  ...
  class Index_one_mindmap_args
    include ::Thrift::Struct, ::Thrift::Struct_Union
    MINDMAP_ID = 1

    FIELDS = {
      MINDMAP_ID => {:type => ::Thrift::Types::I32, :name => 'mindmap_id'}
    }
  ...
  class Index_one_mindmap_result
    include ::Thrift::Struct, ::Thrift::Struct_Union
    SUCCESS = 0

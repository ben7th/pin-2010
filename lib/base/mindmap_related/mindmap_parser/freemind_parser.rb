class FreemindParser < MapFileParser
  ...
  def self.export_v8(mindmap)
    xsltpath = MapFileParser.xslt_file_path("mindpin_to_freemind_8.xslt")
  ...
  def self.export_v9(mindmap)
    xsltpath = MapFileParser.xslt_file_path("mindpin_to_freemind_9.xslt")
    xml = self.xslt_transform_form_xml(mindmap.struct,xsltpath)
    nokogiri_doc = Nokogiri::XML(xml)
  ...
  def self._v9_handle_node_title(nokogiri_doc)
    nokogiri_doc.css("node richcontent[TYPE='NODE'] html body p").each do |n|
      str = n.inner_html
      str.gsub!(/\\n/,"<br>")
      n.inner_html = str
    end

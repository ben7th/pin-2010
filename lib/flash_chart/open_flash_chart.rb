...
def _ofc(width, height, url, set_wmode_transparent=false)
  url = CGI::escape(url)
  out = []
  ...
  swf_url = pin_url_for("pin-user-auth","/flash/open-flash-chart.swf")
  flashvars = {:data=>url}
  params = {:allowscriptaccess=>"sameDomain"}
  params[:wmode] = "transparent" if set_wmode_transparent
#  out << %`
#      <div id="#{div_name}"></div>
#      <script type="text/javascript">
#        swfobject.embedSWF("#{swf_url}","#{div_name}","#{width}","#{height}","9.0.0",null,#{flashvars.to_json},#{params.to_json})
#      </script>
#  `
...
def open_flash_chart_object_str(width, height, url, set_wmode_transparent=false)
  return _ofc(width, height, url, set_wmode_transparent)  
end

def open_flash_chart_object(width, height, url, set_wmode_transparent=false)
  return _ofc(width, height, url, set_wmode_transparent)
end

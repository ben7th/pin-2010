class WorkerManagement
  ...
  class << self
    ...
    def start?(worker_name)
      ...
      state(worker_name) == "正常运行"
    rescue Exception=>ex
      raise ex
    end

    # worker的状态
    def state(worker_name)
      check_worker_param(worker_name)
    ...
    # worker的操作
    def operate(worker_name,operation)
      check_worker_param(worker_name)
